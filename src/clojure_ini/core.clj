...
(defn- parse-line [^java.lang.String s kw trim]
  (if (= (first s) \[)
...
(defn- strip-comment [^java.lang.String s chr allow-anywhere?]
  ...
  (let [kw   (if keywordize? keyword identity)
    ...
    (with-open [r (io/reader in)]
      (->> (line-seq r)
           ...
           (remove (fn [s] (every? #(Character/isWhitespace ^java.lang.Character %) s)))
